...
add_de1_widget "iconik_settings" checkbutton 160 240 {} -text [translate "Show Water level indicator"] -indicatoron true  -font $::font_tiny -bg [theme background] -anchor nw -foreground [theme button_text_light] -variable ::iconik_settings(show_water_level_indicator)  -borderwidth 0 -selectcolor [theme background] -highlightthickness 0 -activebackground [theme background]  -bd 0 -activeforeground [theme button_text_light] -relief flat -bd 0
...
} -width [expr {int(38 * $::globals(entry_length_multiplier))}] -font $::font_tiny  -borderwidth 1 -bg [theme background_highlight]  -foreground [theme button_text_light] -textvariable ::iconik_settings(cleanup_profile) -relief flat  -highlightthickness 1 -highlightcolor [theme button_text_light]

# Screensaver folder
add_de1_text iconik_settings 180 720 -text [translate "Screensaver folder"] -font $::font_tiny -width 300 -fill [theme button_text_light] -anchor "nw"
# The actual content. Here a list of all settings for this plugin
add_de1_widget "iconik_settings" entry 180 780  {
    set ::globals(widget_profile_name_to_save) $widget
    bind $widget <Return> { say [translate {save}] $::settings(sound_button_in); borg toast [translate "Saved"]; iconik_save_settings; hide_android_keyboard}
    bind $widget <Leave> hide_android_keyboard
} -width [expr {int(38 * $::globals(entry_length_multiplier))}] -font $::font_tiny  -borderwidth 1 -bg [theme background_highlight]  -foreground [theme button_text_light] -textvariable ::iconik_settings(saver_dir) -relief flat  -highlightthickness 1 -highlightcolor [theme button_text_light]
