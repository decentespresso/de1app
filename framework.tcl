...
proc create_button { contexts x1 y1 x2 y2 text font backcolor textcolor action } {
	...
	add_de1_text $contexts [expr ($x1 + $x2) / 2.0 ] [expr ($y1 + $y2) / 2.0 ] -text $text -font $font -fill $textcolor -anchor "center" -state "hidden"
	add_de1_button $contexts $action $x1 $y1 $x2 $y2
}

proc create_settings_button { contexts x1 y1 x2 y2 text font backcolor textcolor action_down action_up} {
	rounded_rectangle $contexts  $x1 $y1 $x2 $y2 [rescale_x_skin 80] $backcolor
